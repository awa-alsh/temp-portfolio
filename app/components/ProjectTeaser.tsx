...
export default function ProjectTeaser({ 
  ...
  const MediaContainer = () => (
    <div className="relative w-full md:w-[50%] aspect-video overflow-hidden flex-shrink-0">
      {type === 'video' ? (
        <video
          ref={videoRef}
  ...
  const ContentContainer = () => (
    <div className="flex-1 flex flex-col justify-between min-w-0">
      ...
      <div className="flex items-center space-x-2 mt-4 md:mt-0">
        <span className={`font-medium ${isUnavailable ? 'text-text-secondary' : 'text-accent'}`}>
        ...
        {isUnavailable ? (
          ...
          <svg width="24" height="8" viewBox="0 0 16 8" fill="none" xmlns="http://www.w3.org/2000/svg" className="arrow-icon w-[40px] h-auto">
  ...
  return (
    <div 
      className={`group relative flex flex-col md:flex-row items-stretch gap-4 md:gap-8 bg-secondary ${isUnavailable ? '' : 'cursor-pointer'}`}
    >
      <div className="flex flex-col md:flex-row flex-1 gap-4 md:gap-8 p-4 md:p-6">
        {imageOnRight ? (
          <>
            <ContentContainer />
            <MediaContainer />
